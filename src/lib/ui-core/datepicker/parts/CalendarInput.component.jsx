...
function CalendarInput({ name, value, onClick, hasError }) {
  const iconClass = `flex z-10 leading-snug font-normal absolute text-center absolute bg-transparent rounded text-base items-center justify-center h-7 w-7 right-0 cursor-pointer`;
  const errorClasses = `text-red-300 bg-red-100 border-red-300 
    focus:border-red-300 placeholder-red-600`;
  return (
    <div className="flex w-full max-h-7 h-7">
      <input
        name={name}
        readOnly={true}
        type="text"
