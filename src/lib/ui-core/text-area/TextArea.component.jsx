...
function TextArea(props) {
  const { className, name, icon, label, showCheck, touched, error, autocomplete, ...rest } = props;
  ...
  const fieldClassName = `resize-none py-1 pl-2 placeholder-gray-400 relative bg-white bg-white border border-gray-300 rounded focus:ring-0
    ...
    <div className="relative flex flex-col flex-wrap items-stretch">
      ...
      <div className={`relative flex flex-wrap items-stretch ${className}`}>
        <div className="flex w-full">
          ...
          <textarea className={fieldClassName} name={name} {...rest} />
          {touched && (
            <div className="flex z-10 leading-snug font-normal absolute text-center absolute bg-transparent rounded text-base items-center justify-center h-8 w-8 right-0">
              <Icon
                name={rightIconName}
                className={`fill-current ${hasError ? 'text-red-700' : 'text-green-700'}`}
              />
            </div>
          )}
        </div>
